...
Stories written with Narracode/DD-MM-YYYY_TITLE/
  POETICS.md               your commitments, refusals, attentional dialect
  ATTRIBUTION.md           who wrote what: human author, exact AI model(s), IDE runtime
  STATE.md                 the story's unified working memory (4 epistemic tiers)
  drafts/                  numbered draft files
```

`STATE.md` is the part that preserves continuity. It organizes working memory into four distinct tiers: (1) Story Facts (established events and timeline truths), (2) Author Decisions (your explicit directions and constraints), (3) Model Interpretations (revisable subtext and character interiority hypotheses), and (4) Unresolved Possibilities (open obligations, planted seeds, and next-turn possibilities). Small- to medium-sized stories use `STATE.md` exclusively; only complex multi-chapter epics outgrowing one file split into modular `structural/` directories.
...
Edit any draft directly in your editor. On the next snapshot, the harness diffs your version against the immutable original draft or Git commit, works out what your edits were doing (compressing, sharpening, cutting explanation, changing agency), logs the observations, and proposes updates to `POETICS.md` when a pattern recurs. It proposes; it does not rewrite your poetics unilaterally.

## What is AUTO_MODE?

A full-pipeline automated run with intermediate manual confirmation pauses suspended: initiate, draft each scene or act, synchronize `STATE.md`, and complete with a holistic reflexive critique against `POETICS.md`. It is model- and runtime-agnostic—executing within whichever IDE (Claude Code, Antigravity, Codex) you are running—and logs the exact participating model in `ATTRIBUTION.md`. For the complete history of how AUTO_MODE and the harness evolved, see the [Harness Evolution Archive](https://jhave.github.io/narracode/harness-history.html).
